...
const props = defineProps({
    ...
    genderStats: {
        type: Object,
        default: () => ({ pria: 0, wanita: 0, total: 0, pria_percent: 50, wanita_percent: 50 }),
    },
    usiaStats: {
        type: Array,
        default: () => [],
...
<template>
    <AppLayout title="Dashboard" :role="role">
        ...
        <div class="space-y-6 pb-8">
            <!-- 1. EXECUTIVE GREETING HEADER CARD -->
            <div class="rounded-3xl bg-white border border-slate-200/90 p-6 sm:p-7 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <!-- 3. VISUAL CHARTS & DEMOGRAPHIC SECTION (ALL ROLES) -->
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <!-- CHART 1: Demografi Kelompok Usia (Piramida Generasi) -->
                <div class="rounded-2xl bg-white border border-slate-200/80 p-6 shadow-xs flex flex-col justify-between">
                    <div class="space-y-1 mb-5">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[11px] font-bold text-slate-400">Total: {{ totalUmatCount }} Jiwa</span>
                        </div>
                        <p class="text-xs text-slate-500">Distribusi piramida generasi umat yang terdaftar</p>
                    </div>

                    <div class="space-y-4 flex-1">
                        <div v-for="u in usiaStats" :key="u.label" class="space-y-1.5">
                            <div class="flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2 text-slate-700 font-semibold">
                                    <i :class="['fa-solid', u.icon, u.textColor]"></i>
                                    <span>{{ u.label }}</span>
                                </div>
                                <span class="font-bold text-slate-900">{{ u.count }} <span class="text-slate-400 font-normal">({{ u.percent }}%)</span></span>
                            </div>
                            <div class="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                    :class="['h-full rounded-full transition-all duration-500', u.color]"
                    ...
                    <div class="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                        <span>Pembaruan otomatis real-time</span>
                        <Link :href="`${basePrefix}/statistik`" class="text-amber-600 hover:text-amber-700 font-bold">
                            Lihat Detail &rarr;
                        </Link>
                    </div>
                </div>

                <!-- CHART 2: Komposisi Gender Laki-laki vs Perempuan -->
                <div class="rounded-2xl bg-white border border-slate-200/80 p-6 shadow-xs flex flex-col justify-between">
                    <div class="space-y-1 mb-4">
                        <h3 class="text-sm font-bold text-slate-900 flex items-center gap-2">
                            <i class="fa-solid fa-venus-mars text-indigo-600"></i>
                            <span>Komposisi Gender Umat</span>
                        </h3>
                        <p class="text-xs text-slate-500">Perbandingan jenis kelamin warga paroki</p>
                    </div>

                    <!-- Donut / Gauge Visualizer -->
                        ...
                        <div class="relative w-36 h-36 flex items-center justify-center">
                            ...
                            <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                ...
                                <path
                                    ...
                                    :stroke-dashoffset="100 - (genderStats.pria_percent || 50)"
                                    stroke-width="4.5"
                            ...
                            <div class="absolute text-center">
                                <span class="text-xl font-black text-slate-900 leading-none">{{ genderStats.total || totalUmatCount }}</span>
                                <span class="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Jiwa</span>
                            </div>
                        </div>

                        <!-- Gender Badges -->
                        <div class="grid grid-cols-2 gap-3 w-full mt-4">
                            <div class="p-3 rounded-xl bg-sky-50/70 border border-sky-100 flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-sky-500 text-white flex items-center justify-center text-sm shrink-0">
                                    <i class="fa-solid fa-mars"></i>
                                </div>
                                <div class="min-w-0">
                                    <span class="block text-[11px] font-bold text-sky-900">Laki-laki</span>
                                    <span class="block text-xs font-black text-sky-700">{{ genderStats.pria }} ({{ genderStats.pria_percent }}%)</span>
                                </div>
                            </div>
                            <div class="p-3 rounded-xl bg-rose-50/70 border border-rose-100 flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-rose-500 text-white flex items-center justify-center text-sm shrink-0">
                                    <i class="fa-solid fa-venus"></i>
                                </div>
                                <div class="min-w-0">
                                    <span class="block text-[11px] font-bold text-rose-900">Perempuan</span>
                                    <span class="block text-xs font-black text-rose-700">{{ genderStats.wanita }} ({{ genderStats.wanita_percent }}%)</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-3 border-t border-slate-100 text-center text-[11px] text-slate-400">
                        Rasio seimbang demografi pastoral
                    </div>
                </div>

                <!-- CHART 3: Sebaran Umat per Komunitas Basis / Status Kawin -->
                <div class="rounded-2xl bg-white border border-slate-200/80 p-6 shadow-xs flex flex-col justify-between">
                    <div class="space-y-1 mb-5">
                        <div class="flex items-center justify-between">
                            <h3 class="text-sm font-bold text-slate-900 flex items-center gap-2">
                                <i class="fa-solid fa-layer-group text-emerald-600"></i>
                                <span>Status Perkawinan & KUB</span>
                            </h3>
                            <span class="text-[11px] font-bold text-slate-400">Keluarga</span>
                        </div>
                        <p class="text-xs text-slate-500">Status perkawinan & sebaran teritori aktif</p>
                    </div>

                    <div class="space-y-4 flex-1">
                        <!-- Status Kawin Progress -->
                        <div v-for="k in statusKawinStats" :key="k.label" class="space-y-1.5">
                            <div class="flex items-center justify-between text-xs">
                                <span class="text-slate-700 font-semibold truncate">{{ k.label }}</span>
                                <span class="font-bold text-slate-900">{{ k.count }} <span class="text-slate-400 font-normal">({{ k.percent }}%)</span></span>
                            </div>
                            <div class="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                    :class="['h-full rounded-full transition-all duration-500', k.color]"
                                    :style="{ width: `${Math.max(4, k.percent)}%` }"
                                ></div>
                            </div>
                        </div>

                        <!-- Top KUB Quick Pills if Available -->
                        <div v-if="sebaranStats.length > 0" class="pt-2">
                            <span class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Komunitas KUB:</span>
                            <div class="flex flex-wrap gap-1.5">
                                <span
                                    v-for="s in sebaranStats.slice(0, 4)"
                                    :key="s.id"
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200 text-[11px] font-semibold text-slate-700"
                                >
                                    <i class="fa-solid fa-people-roof text-amber-600"></i>
                                    <span class="truncate max-w-[120px]">{{ s.nama }}</span>
                                    <span class="font-bold text-amber-700">({{ s.count }})</span>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                        <span>Basis data pastoral terpusat</span>
                        <Link :href="`${basePrefix}/kk-katolik`" class="text-amber-600 hover:text-amber-700 font-bold">
                            Data KK &rarr;
                        </Link>
                    </div>
                </div>
            ...
            <!-- 4. BOTTOM GRID: LATEST UMAT & SAKRAMEN DETAILS -->
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <!-- Latest Umat List -->
